...
def load_df_compound():
  ...
  while True:
      res = requests.post(url, json={"query": market_tx, "variables": {"hour": hour, "id": last_id}})
  ...
  df_compound = pd.DataFrame(market_data, columns=["loan_asset", "hours", "supplyApy", "borrowApy", "totalSupplyUSD", "totalBorrowUSD"])
  ...
  df_compound[columns_to_convert] = df_compound[columns_to_convert].astype(float)
  df_compound["borrowApy"] = df_compound["borrowApy"]/100.
  df_compound["supplyApy"] = df_compound["supplyApy"]/100.
  df_compound['utilization'] = df_compound['totalBorrowUSD'] / df_compound['totalSupplyUSD']

  df_compound = df_compound[['date', 'loan_asset', 'supplyApy', 'borrowApy', 'utilization', 'totalSupplyUSD', 'totalBorrowUSD']]
